...
class ContextMaker():
    ...
    def get_pmap_by_grp(self, srcfilter, group):
        ...
        pmaker = PmapMaker(self, srcfilter, src_mutex, rup_indep)
        if src_mutex:  # one source at the time
            for src, s_sites in srcfilter(group):
                pmaker.srcs = [src]
                pmaker.s_sites = s_sites
                try:
                    poemap = pmaker.make(gids, rup_data, dists, calc_times)
                    if poemap:
...
class PmapMaker():
    ...
    """
    def __init__(self, cmaker, srcfilter, src_mutex, rup_indep):
        vars(self).update(vars(cmaker))
        self.cmaker = cmaker
        self.srcfilter = srcfilter
        self.src_mutex = src_mutex
        self.rup_indep = rup_indep
        self.fewsites = (len(srcfilter.sitecol.complete) <=
                         cmaker.max_sites_disagg)
    ...
    def make(self, gids, rup_data, dists, calc_times):
        """
        ...
        with self.cmaker.mon('iter_ruptures', measuremem=False):
            self.mag_rups = [
                (mag, list(rups)) for src in self.srcs
